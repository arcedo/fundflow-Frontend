...
function ProjectThumb({ projectId, projectName, projectUrl, projectCreator, creatorUrl, projectCategory, projectType, likes, dislikes, fundedPercentage, views }) {
    ...
    const handleOptionClick = (action) => {
        ...
        switch (action) {
            case "Edit":
                navigate(`/projects/${projectUrl}/edit`);
                break;
            case "Delete":
    ...
    return (
        <div className="flex flex-col group/project" ref={optionsRef}>
            ...
            <div className="relative flex flex-col justify-center items-center bg-gradient-to-r from-primary to-secondary h-44 sm:h-60 w-full rounded-md">
                <div className="flex gap-0.5 items-center absolute font-dmsans backdrop-blur top-3 right-3 z-30 py-2 px-3 bg-555/55 shadow-sm text-white text-sm font-bold rounded-full group-hover/project:translate-x-1.5 group-hover:shadow-md group-hover/project:-translate-y-1.5 transition-all duration-200 lowercase">
                    <p>{projectCategory}</p>
                    <hr className="rotate-90 w-4" />
                    <img className={`w-5 inline-block ${projectType === "fund" ? 'rotate-90' : ''}`} src={projectType === "fund" ? fundsWhite : collaboratorsWhite} alt="Project Type Icon" />
                </div>
