    ...
    return (
        <section className="flex justify-center items-center fade-in w-full">
            <div className="w-full">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-8 w-full">
                    {usersFound && !usersFound.message ? usersFound.map((user, index) => {
                        const delay = index * 0.05;
                        return (
                            <div key={user.user} style={{ animationDelay: `${delay}s` }} className="fade-in">
                                <UserThumb user={user} />
                            </div>
                        );
                        ...
                        <p className="text-black text-opacity-75 font-dmsans font-semibold text-xl">{onEmptyMessage ?? 'No users found'}</p>
                    </div>
                    }
                    {Array.from({ length: placeholdersNeeded }).map((_, index) => {
                        const delay = (usersFound.length + index) * 0.05;
                        return (
                            <div key={`placeholder-${usersFound.length + index}`} style={{ animationDelay: `${delay}s` }} className="fade-in flex flex-col items-center justify-center gap-4">
